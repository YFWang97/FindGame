...
SDL_Texture* load_text(const char* text, bool red) {

    SDL_Surface* surface;
	if (red) {
		surface = TTF_RenderText_Solid(gFont, text, {255, 80, 0});
	} else { 
		surface = TTF_RenderText_Solid(gFont, text, {0, 0, 0});
	}
